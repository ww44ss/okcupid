...
Overall the number of men, `r n_males`, is about `r round(n_males/n_females, 1)` times greater than the number of females, `r n_females`. 
...
```{r echo=1:5, fig.align='center', fig.height=5,fig.width=7.5, message = FALSE}
    ...
    n.color <- length(unique(religion_count$religious_affil))
    getPalette = colorRampPalette(brewer.pal(9, "Paired"))
    
    
    ## plot
    p <- ggplot(religion_count, aes(religious_affil, weight = group_count)) +
        geom_bar(aes(fill = religious_affil), alpha = 0.9) +
        ggtitle("OkCupid ") + facet_grid(sex~.) +
        coord_flip() + 
        scale_fill_manual(values = getPalette(n.color)) + 
        ylab("number") + 
...
### How des driking change with age for men and women?
