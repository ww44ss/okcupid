...
```{r echo=FALSE, fig.align='center', fig.height=4,fig.width=7.5}
    ...
    ## clean religion data into broad category
    bnG <- filter(profiles, !is.na(age), !is.na(sex), age > 18, age < 80) %>% 
            as_data_frame %>%
            select(sex, age)

    bnG$sex<- bnG$sex %>% gsub("m", "male", .) %>% gsub("f", "female", .)
    

    bnG <- bnG %>% mutate(age_group = 2 + 4 * floor(age/4))
    bnG$age_group <- bnG$age_group %>% as.factor
    
    ## group the data 
    age_count <- group_by(bnG[,c("age_group", "sex")], age_group, sex) %>% summarize(n_age = n())

    
    sex_count <- group_by(bnG[,c("sex")], sex) %>% summarize(n_sex = n())
    ...
    n.color <- length(unique(bnG$age_group))
    ...
    p <- ggplot(analyzed, aes(x = age_group, y = 100*freq, color = sex, group = sex)) +
        geom_line(size = 2) +
...
p <- ggplot(analyzed_wide2, aes(x = age_group, y = male_cummul, group = sex.x)) +
        geom_line(size = 1.5, color = "#DD9834") +
        geom_line(aes(y = female_cummul, group = sex.y), size = 1.5, color = "#3498DD") +
...
p <- ggplot(analyzed_wide, aes(x = age_group, y = freq.x, group = sex.x)) +
        geom_line(size = 1.5, color = "#DD9834") +
        geom_line(aes(y = freq.y, group = sex.y), size = 1.5, color = "#3498DD") +
        ylab("cummulative percent") +
        xlab("age") +
        ggtitle("cummulative populations by age")
