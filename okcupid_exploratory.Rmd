...
The data comprise:  
* `r nrow(profiles)` observations.  
* `r ncol(profiles)` variables.  
...
```{r echo=TRUE, fig.align='center', fig.height=3,fig.width=9, tidy=TRUE}
    ...
    male_deciles <- male_summary$age %>% quantile(c(.25, .5, .75, .95))
        
    female_summary <- cleaned %>% filter(sex == "female") %>% select(age)
    female_deciles <- female_summary$age %>% quantile(c(.25, .5, .75, 0.95))
```

For reference 50% of the population lies in the narrow range of ages between `r age_quantiles[1]` and `r age_quantiles[2]` years old. The long tail implies 95% of the population is `r age_quantiles[3]` or younger (for men and women the 95th quantile is `r male_deciles[4]` years and `r female_deciles[4]` years, respectively). 
...
The table shows the ratio of men's and women's mean incomes is `r 100* ((analyzed$avg_income[1]/analyzed$avg_income[2]) %>% round(2))`%, which is [consistent with other findings](http://www.iwpr.org/initiatives/pay-equity-and-discrimination). Given the young and progressive OkCupid population, the result is both surprising and a little disappointing.  
...
```{r echo=FALSE, fig.align='center', fig.height=5,fig.width=7.5, message = FALSE, tidy=TRUE}
...
## clean data into broad category
    ...
    cleaned <- mutate(cleaned, age_range = 5 + 10 * age %/% 10 )
...
```{r echo = FALSE, fig.align = 'center', fig.height = 4 , fig.width = 7.5 , message = FALSE}
    ...
    income <- group_by(cleaned[,c("drinks", "income", "sex")], income, sex) %>% summarize(NN = n())
