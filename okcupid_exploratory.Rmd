...
MFP <- data_frame("variable" = colnames(profiles), "MFP" = rep(NA, length(names(profiles))))
    ...
    profiles_f <- profiles %>% filter(job != "other")
    ## loop thru all variable names in the profile
    for (ii in 1:(names(profiles) %>% length)) {
...
These outcomes translate to an story (that is perhaps not all to surprising, but still interesting):
...
**The female MPH of the OkCupid** is a *white, straight, single, female,* who is *25* years old. She *graduated from college/university*, works as a *student*. er body type is *average* and is *5\' 4\"* tall. She eats *mostly anything*, drinks *socially*, but *never* takes drugs. She lives in *San Francisco*, *doesn't have kids* and *likes dogs and cats*. Her astrologic sign is *gemini and it’s fun to think about*, and when asked about religion respnds *agnosticism but not too serious about it*. She speaks only *english*. 
